...
// Animated Step 02 - Scrolling conversation showing 2 messages at a time
function Step02Visual() {
  const [currentIndex, setCurrentIndex] = useState(0);

  // Full conversation as individual messages
  const messages = [
    ...
    { id: 2, type: "ai", text: "Supratau, kiek šiuo metu apsiemate projektų ir kiek norėtumėte apsiimti?" },
    { id: 3, type: "customer", text: "Per mėnesį atliekam apie 10 projektų, bet norėtusi gauti bent dvigubai." },
  ...
  useEffect(() => {
    ...
    }, 2800); // Show each message for 2.8 seconds

    return () => clearInterval(interval);
  }, []);

  // Show current message and previous message (2 messages total)
  const visibleMessages = currentIndex === 0 
    ? [messages[0]] // First message shows alone
    : [messages[currentIndex - 1], messages[currentIndex]]; // Show previous and current
...
// Animated Step 03 - Alternating notifications
...
// Animated Step 04 - Nurturing sequence
...
export default function HowItWorksApple() {
  ...
  const steps = [
      ...
      visual: (
        <div className="relative bg-white dark:bg-card rounded-2xl border border-border p-6 md:p-8 h-full flex items-center justify-center min-h-[300px]">
          <div className="relative w-full max-w-[240px] aspect-square">
            ...
            {/* Dashed line - Top (AI → Facebook, flows upward) */}
            ...
            {/* Dashed line - Left (AI → Instagram, flows left) */}
            ...
            {/* Dashed line - Right (AI → SMS, flows right) */}
            ...
            {/* Dashed line - Bottom (AI → Email, flows downward) */}
    ...
    },
    {
      ...
      description: "Jei klientas sako \"man reikia laiko\", sistema jį \"šildo\" informatyviu turiniu ir po kurio laiko vėl inicijuoja pardavimų pokalbį.",
      visual: <Step04Visual />,
    },
  ];

  return (
    <section ref={ref} className="pt-12 md:pt-16 pb-12 md:pb-16 px-6 lg:px-12">
      <div className="max-w-5xl mx-auto">
        <motion.h2 
          initial={{ opacity: 0, y: 20 }}
          animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
          transition={{ duration: 0.6 }}
          className="text-2xl md:text-3xl lg:text-4xl font-bold text-center mb-12 tracking-tight"
        >
          Kaip veikia AI<br className="md:hidden" /> pardavimų sistema?
        </motion.h2>
        ...
        <div className="flex justify-center mt-24 md:mt-16 mb-10 md:mb-0">
          <Link href="/webinar">
            <Button 
              size="lg" 
              variant="default"
